...
public class DaysDetailsDAO implements DaysDetailsDAOInterface {
	...
	private static final String selectAllTrainingDaysByDayId = "select * from days_master where day_id IN (%s)";
	@Autowired
	...
	public List<DaysDetails> getAllSelectedDaysByDayId(List<Integer> dayIdList) {
		String inSql = String.join(",", Collections.nCopies(dayIdList.size(), "?"));

		List<DaysDetails> selectedDaysList = jdbcTemplate.query(
				String.format(selectAllTrainingDaysByDayId, inSql),
				new DaysDetailsRowMapper(jdbcTemplate), dayIdList.toArray());
